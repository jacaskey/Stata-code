*!Version 2.3.5 3jul14 (By Judson Caskey)
...
* 2.3.4: Removed option for pweight because the _robust routine behaves strangely with them
*
...
program define cgmreg, eclass byable(onecall) sortpreserve
	...
	syntax varlist(fv) [if] [in] [aweight fweight iweight /], Cluster(varlist) [*]
		...
		if mod(`included',2)==0 { /* even number */
			local `plusminus' "-"
                ...
                *sub_robust if `touse' `weight', groupvar(`groupvar') xxinv(`xxinv') plusminus(``plusminus'') resid(`resid') running_sum(`running_sum') touse(`touse')
                sub_robust if `touse' `weight', groupvar(`groupvar') xxinv(`xxinv') plusminus(``plusminus'') resid(`resid') running_sum(`running_sum') touse(`touse') vl(`varlist')
...
prog define sub_robust
	...
	syntax [if] [in] [aweight fweight iweight pweight /] , groupvar(string) xxinv(string) plusminus(string) resid(string) running_sum(string) touse(string) vl(string)
...
*/	
	...
	/*
	di "plusminus: `plusminus'"
	di "nrows:  `=`nrows''"

	di "running_sum"
	matrix list `running_sum'
	di "m - before"
	matrix list `m'
	*/

	qui _robust `resid' `weight' `if' `in', v(`m') minus(`=`nrows'') cluster(`groupvar') `zw'
	*di "m - after"
	*matrix list `m'
	mat `running_sum' = `running_sum' `plusminus' `m'

	*qui reg `vl' `if' `in' `weight', cluster(`groupvar') `options'
	*mat `running_sum' = `running_sum' `plusminus' e(V)

	*di "running_sum plusminus m"
	*mat li `running_sum'
end
