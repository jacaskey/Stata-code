*!Version 2.3.2 19feb14 (By J. Caskey)
...
*! 24Aug10 (By Judson Caskey - Changed e(cmd) to logit so it works with mfx
*! 27Aug10 (By Judson Caskey - Added e(rules) and e(opt) so it works with predict
...
program define cgmlogit, eclass byable(onecall) sortpreserve
	...
	tempname b b2 bT running_sum Bigmat element elementM groupvar rows cols chi2 depname e_df_m e_p numcvars numvars S numsubs e_r2_p c n ///
		e_ll_0 e_ll e_N_cdf e_N_cds oV optmax tV mRules b2 v2 noomit
	...
	*taking inductive approach
	...
	matrix `mRules'=e(rules)
    * If max option set, replace diagonals with maximum of 'vanilla', White, or clustered errors

    if "``optmax''"=="max" {
        qui logit `varlist' if `touse' `weight', `options'
        matrix `oV'=e(V)
		
    
        qui logit `varlist' if `touse' `weight', `options' robust
        matrix `tV'=e(V)
        ...
        foreach clusvar in `cluster' {
            qui logit `varlist' if `touse' `weight', `options' cluster(`clusvar')
            matrix `tV'=e(V)
            forvalues k=1(1)``numvars'' {
                if `tV'[`k',`k']>`oV'[`k',`k'] {
                    matrix `oV'[`k',`k']=`tV'[`k',`k']
