...
capture program drop bking
program define bking, rclass
	version 7.0

syntax varlist(ts) [if] [in], plo(real) phi(real) [K(int 12)]  /* GENerate(string) */
	...
	if `plo' < 2     {
	di in red "Lower bound (plo) must be greater than 2"
		error 198
		}
	if `k' < 1     {
	...
	tempname omubar omlbar avec theta

	scalar `omubar' = 2*_pi/`plo'
	scalar `omlbar' = 2*_pi/`phi'

	matrix `avec' = J(`k'+1,1,0)
	matrix `avec'[1,1] = (`omubar'-`omlbar')/_pi

	forvalues k=1(1)`k' {
		`avec'[`k'+1,1] = (sin(`k'*`omubar')-sin(`k'*`omlbar'))/(`k'*_pi)
		}
...
* F will be aligned with original timeseries obs
    	...
    	local vars
    	forv i = 1/`c' {
		local v : word `i' of `veel'
    		local varnam = "F`v'"
    		local vars "`vars' `varnam'"
    	}
	...
	matrix fXtmp = `F'
 	...
 	forv j = 1/`c' {
		tempvar y`j'
       	gen `type' `y`j'' = matrix(`F'[_n, `j']) in 1/`r'
       	rename `y`j'' ``j''
       	}
*	svmat double `F', names(`generate')
